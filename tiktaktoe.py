...
def next_to_mark(cells: list, mark: str, count_next_to_marks: int) -> list[int]:
...
def set_mark_position(user_mark: str, game_area: list[int], position: int) -> bool:
	"""Установка маркера игрока в соответствующую позицию. Иначе вернет False"""

	if not (0 <= position <= 10 * 10):
		return False
...
def five_mark_check(temp: str, mark: str) -> bool:
...
def full_check(game_area : list) -> bool:
	"""Проверка на ничью"""
	if game_area.count('X') == game_area.count('O') and game_area.count("X") > 0 and game_area.count("O") > 0:
		return True
	return False
